 ...
 */
package Algorithm.KnapsackGenetic;
...
public class KnapsackProblem {
    ...
    private double knapsack_size_capacity;
    private double knapsack_weight_capacity;
    private double prob_crossover = Setting.knapsack_crossover;
    private double prob_mutation = Setting.knapsack_mutation;
    private double total_fitness_of_generation = 0;
    private ArrayList<Double> value_of_items = new ArrayList<Double>();
    private ArrayList<Double> weight_items = new ArrayList<Double>();
    private ArrayList<Double> size_items = new ArrayList<Double>();
    ...
    public KnapsackProblem(ArrayList<PickItem> pickingList, double size_capacity, double weight_capacity) {
        this.knapsack_size_capacity = size_capacity;
        this.knapsack_weight_capacity = weight_capacity;
        this.number_of_items = pickingList.size();

        for (PickItem item : pickingList) {
            value_of_items.add(1.0);

            String sku = item.getProductSKU();
            Product product = ProductDAO.getProduct(sku);
            if (product != null) {
                weight_items.add(product.getWeight() * item.getNoOfCarton());
                size_items.add(product.getWidth() * product.getHeight() * product.getLength() * item.getNoOfCarton());
            } else {
                weight_items.add(Setting.unknown_product_weight * item.getNoOfCarton());
                size_items.add(Setting.unknown_product_length * Setting.unknown_product_width * Setting.unknown_product_height * item.getNoOfCarton());
            }
        }
    }
...
//        for (int i = 0; i < Setting.knapsackPopulationSize; i++) {
...
//        System.out.println("Fitness score of best solution of initial generation: " + this.best_fitness_of_generation.get(0));
        ...
        if (Setting.knapsackGenerationSize > 1) {
            makeFurtherGenerations();
        }
        
        ArrayList<Integer> pickingSack = new ArrayList<Integer>();
        String result = getOptimalList();
        
        for(int i = 0; i < number_of_items; i++){
            pickingSack.add(Integer.parseInt(result.charAt(i) + ""));
        }
    ...
    private void makeFurtherGenerations() {
        ...
        for (int i = 1; i < Setting.knapsackGenerationSize; i++) {

            // Check for stopping criterion
            if ((Setting.knapsackGenerationSize > 4) && (i > 4)) {
                ...
                if (a == b && b == c) {
//                    System.out.println("\nStop criterion met");
                    Setting.knapsackGenerationSize = i;
                    break;
                }
            }
            ...
            // Breed population
            for (int j = 0; j < Setting.knapsackPopulationSize / 2; j++) {
            ...
            for (int k = 0; k < Setting.knapsackPopulationSize; k++) {
...
//            }
//            System.out.println("Population:");
//            for (int l = 0; l < Setting.knapsackPopulationSize; l++) {
//                System.out.println((l + 1) + " - " + this.population.get(l));
//            }
//
//            // Output fitness summary
//            System.out.println("\nFitness:");
//            for (int m = 0; m < Setting.knapsackPopulationSize; m++) {
...
//        System.out.println("\nOptimal list of items to include in knapsack: ");
        ...
        for (int z = 0; z < Setting.knapsackGenerationSize - 1; z++) {
...
//            }
//        }
        return this.best_solution_of_generation.get(best_gen);
    }
    ...
    private void breedPopulation() {
        ...
        if (Setting.knapsackPopulationSize % 2 == 1) {
    ...
    private double getMeanFitness() {
        ...
        mean_fitness = total_fitness / Setting.knapsackPopulationSize;
        return mean_fitness;
    }
    ...
    private double evalGene(String gene) {
        ...
        for (int j = 0; j < number_of_items; j++) {
            ...
            if (c == '1') {
                totalWeight += weight_items.get(j);
                totalSize += size_items.get(j);
                total_value = total_value + value_of_items.get(j);
            }
        }
        // Check if gene's total weight is less than knapsack capacity
        differenceWeight = knapsack_weight_capacity - totalWeight;
        differenceSize = knapsack_size_capacity - totalSize;
        if (differenceWeight >= 0 && differenceSize >= 0) {
    ...
    /**
     * Makes a population by filling population ArrayList with strings of length
     * number_of_items, each element a gene of randomly generated chromosomes
     * (1s and 0s)
     */
